'use client';

import { useState } from 'react';
import { addProjectPart } from '@/app/actions';
import { Plus, Search } from 'lucide-react';

interface Part { id: number; name: string; sku: string | null; priceFleet: number | null; price: number; quantity: number; }

export default function AgregarParteForm({ projectId }: { projectId: string }) {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<Part[]>([]);
  const [selected, setSelected] = useState<Part | null>(null);
  const [qty, setQty] = useState(1);
  const [unitPrice, setUnitPrice] = useState(0);
  const [loading, setLoading] = useState(false);

  async function search(q: string) {
    setQuery(q);
    if (q.length < 2) { setResults([]); return; }
    const res = await fetch(`/api/parts?q=${encodeURIComponent(q)}`);
    const data = await res.json();
    setResults(data.slice(0, 8));
  }

  function selectPart(p: Part) {
    setSelected(p);
    setUnitPrice(p.priceFleet ?? p.price);
    setResults([]);
    setQuery(p.name);
  }

  return (
    <form
      action={async (fd) => {
        if (!selected) return;
        setLoading(true);
        await addProjectPart(fd);
        setSelected(null);
        setQuery('');
        setQty(1);
        setUnitPrice(0);
        setLoading(false);
      }}
      className="space-y-3"
    >
      <p className="text-sm font-semibold text-gray-700">Agregar parte</p>
      <input type="hidden" name="projectId" value={projectId} />
      {selected && <input type="hidden" name="partId" value={selected.id} />}

      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
        <input
          className="input-field pl-9 text-sm"
          placeholder="Buscar parte por nombre o SKU..."
          value={query}
          onChange={(e) => { setSelected(null); search(e.target.value); }}
          autoComplete="off"
        />
        {results.length > 0 && (
          <ul className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
            {results.map((p) => (
              <li key={p.id}>
                <button type="button" onClick={() => selectPart(p)} className="w-full text-left px-4 py-2 hover:bg-gray-50 text-sm">
                  <span className="font-medium">{p.name}</span>
                  {p.sku && <span className="ml-2 text-xs text-gray-400">SKU: {p.sku}</span>}
                  <span className="ml-2 text-xs text-gray-400">Stock: {p.quantity}</span>
                </button>
              </li>
            ))}
          </ul>
        )}
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="label-field">Cantidad</label>
          <input name="quantity" type="number" min="1" value={qty} onChange={(e) => setQty(parseInt(e.target.value))} className="input-field text-sm" />
        </div>
        <div>
          <label className="label-field">Precio unitario</label>
          <input name="unitPrice" type="number" step="0.01" min="0" value={unitPrice} onChange={(e) => setUnitPrice(parseFloat(e.target.value))} className="input-field text-sm" />
        </div>
        <div className="flex items-end">
          <button type="submit" disabled={!selected || loading} className="btn-primary w-full text-sm">
            <Plus className="h-4 w-4" /> {loading ? '...' : 'Agregar'}
          </button>
        </div>
      </div>
      {selected && (
        <input type="hidden" name="notes" value="" />
      )}
    </form>
  );
}
