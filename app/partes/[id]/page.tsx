...
export default async function ParteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('Error in ParteDetailPage:', error)
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 p-4 rounded-md">
          <h3 className="text-red-800 font-bold">Error al cargar la parte</h3>
          <p className="text-red-600 text-sm">{error.message || 'Error interno del servidor'}</p>
          <Link href="/partes" className="mt-2 inline-block text-xs text-red-700 underline">Volver al inventario</Link>
        </div>
      </div>
    )
  }
